...
>
> แหล่งอ้างอิง: [GitHub REST API — repos](https://docs.github.com/en/rest/repos/repos), [Stripe API — customers](https://docs.stripe.com/api/customers)

<Callout title="จำลองเพื่อผู้บริโภค ไม่ใช่เพื่อ schema">
resource คือคำศัพท์สาธารณะของ API ดังนั้นให้ปั้นตามวิธีที่ client คิด ไม่ใช่ตามวิธีที่คุณวางตารางไว้ resource หนึ่งตัวจะ join หลายตาราง หรือตารางเดียวจะโผล่ออกมาเป็นสอง resource ก็ไม่ผิดอะไร ขอแค่แยก contract ออกจาก storage ให้ได้
</Callout>
...
export const quiz = [
  ...
  {
    q: 'ข้อความใดเกี่ยวกับ resource และตารางใน database ที่ถูกต้อง?',
    options: [
  ...
  },
  {
    ...
    options: [
      'match กับคอลัมน์เดียวพอดี',
      'เป็นสิ่งที่ client ดึงหรือแก้ไขในฐานะหน่วยเดียว',
      'ใหญ่ที่สุดเท่าที่จะทำได้',
      'ไม่มีความสัมพันธ์กับอะไรเลย',
    ],
    answer: 1,
  },
];
